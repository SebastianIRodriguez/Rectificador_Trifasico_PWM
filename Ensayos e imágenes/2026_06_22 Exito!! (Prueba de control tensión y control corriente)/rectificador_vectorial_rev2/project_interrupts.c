...
float ki_control_i = 5;

float Vbus_sp = 50;    // Antes 40
...
float kp_control_v = 0.01;
float ki_control_v = 0;
...
// Adquisicion de senales simuladas
    ...
    if ((49 < red_trifasica.f) && (red_trifasica.f < 51)) //  && estado_convertidor >= CONVERTER_STARTING_KP
    {
        // ******************** Control tension ******************** //
        ...
        pi_vbus.Ki = (estado_convertidor >= CONVERTER_STARTING_KP) ? ki_control_v : 0;
...
CONVERTER_STATE run_converter_mef(PLL_VARS grid, float vbus)
{
    ...
    static int cuenta_precarga = 0;
    static int cuenta_aux = 0;
    ...
    case CONVERTER_RISING_VBUS:
    {
        ...
        // Esperamos medio segundo
        if ((cuenta_aux > 5000) && (vbus > Vbus_sp * 0.95) && (vbus < Vbus_sp * 1.05))
        {
            // Transicionamos
            cuenta_aux = 0;
            CDC1_CLOSE;
