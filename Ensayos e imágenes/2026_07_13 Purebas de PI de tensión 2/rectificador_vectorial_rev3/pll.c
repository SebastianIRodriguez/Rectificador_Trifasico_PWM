...
// #define SIM_ADQ 1 // Indica si el PLL utiliza datos reales o simulados
...
int show_phase = 0;              // Elije exteriorizar la fase simulada y la fase que percibe el PLL
...
SIGNAL_3Ph SGN_3th_V;            // Contiene la señal de tercer armonico simulada
...
SEQ_CORE NSC_U3;                          // Core de componente de tercer armonico
SEQ_CORE PSC_U3;                          // Core de componente de tercer armonico
SEQ_CORE PSC_U5;                          // Core de componente de quinto armonico positivo
SEQ_CORE NSC_U5;                          // Core de componente de quinto armonico negativo
SEQ_CORE PSC_U5;
SEQ_CORE PSC_U7;                          // Core de componente de septimo armonico positivo
SEQ_CORE NSC_U7;                        // Core de componente de septimo armonico negativo
...
float EPLL_u1 = 200;                     // Variable de ajuste para los cores (amplitud) - 200
float EPLL_u2 = 5000;                    // Antes 12000                   // Ganancia del estimulador, una constante que se ajusta para el funcionamiento del estimador de frecuencia, originalmente 20
float EPLL_u3 = 200;                     // Variable de ajuste para los cores (fase) - 200
...
#if BUILD >= BUILD_1
        // Cores
        SEQ_CORE_init(&PSC_U1, 1, 1, EPLL_u1, EPLL_u3, TWO_PI * F_NOM, time_step);  // Fundamental
        SEQ_CORE_init(&NSC_U1, 1, -1, EPLL_u1, EPLL_u3, TWO_PI * F_NOM, time_step); // Fundamental secuencia negativa
        SEQ_CORE_init(&NSC_U3, 3, -1, EPLL_u1, EPLL_u3, TWO_PI * F_NOM, time_step);   // Tercer armonico, secuencia negativa
        SEQ_CORE_init(&PSC_U3, 3, 1, EPLL_u1, EPLL_u3, TWO_PI * F_NOM, time_step);   // Tercer armonico, secuencia positiva
        SEQ_CORE_init(&NSC_U5, 5, -1, EPLL_u1, EPLL_u3, TWO_PI * F_NOM, time_step);  // Quinto armonico, secuencia negativa
        SEQ_CORE_init(&PSC_U5, 5, 1, EPLL_u1, EPLL_u3, TWO_PI * F_NOM, time_step);  // Quinto armonico, secuencia positiva
        SEQ_CORE_init(&PSC_U7, 7, 1, EPLL_u1, EPLL_u3, TWO_PI * F_NOM, time_step);   // Septimo armonico, secuencia positiva
        SEQ_CORE_init(&NSC_U7, 7, -1, EPLL_u1, EPLL_u3, TWO_PI * F_NOM, time_step);   // Septimo armonico, secuencia positiva
...
#if BUILD >= BUILD_0
    // ****************************** Adquisicon analogica de datos (se toman los parametros)
...
// ************************************************************ Aplicacion del PLL ************************************************************
    ...
    VPLL[0] = PSC_U1.out[0] + U1N_COMP_ENABLE * NSC_U1.out[0] + PSC_U3.out[0] + NSC_U3.out[0] + PSC_U5.out[0] + U5_COMP_ENABLE * NSC_U5.out[0] + U7_COMP_ENABLE * PSC_U7.out[0] + NSC_U7.out[0];
    VPLL[1] = PSC_U1.out[1] + U1N_COMP_ENABLE * NSC_U1.out[1] + PSC_U3.out[1] + NSC_U3.out[1] + PSC_U5.out[1] + U5_COMP_ENABLE * NSC_U5.out[1] + U7_COMP_ENABLE * PSC_U7.out[1] + NSC_U7.out[1];
    VPLL[2] = PSC_U1.out[2] + U1N_COMP_ENABLE * NSC_U1.out[2] + PSC_U3.out[2] + NSC_U3.out[2] + PSC_U5.out[2] + U5_COMP_ENABLE * NSC_U5.out[2] + U7_COMP_ENABLE * PSC_U7.out[2] + NSC_U7.out[2];

    //VPLL[0] = PSC_U1.out[0] + U1N_COMP_ENABLE * NSC_U1.out[0] + U5_COMP_ENABLE * NSC_U5.out[0] + U7_COMP_ENABLE * PSC_U7.out[0];
    //VPLL[1] = PSC_U1.out[1] + U1N_COMP_ENABLE * NSC_U1.out[1] + U5_COMP_ENABLE * NSC_U5.out[1] + U7_COMP_ENABLE * PSC_U7.out[1];
    //VPLL[2] = PSC_U1.out[2] + U1N_COMP_ENABLE * NSC_U1.out[2] + U5_COMP_ENABLE * NSC_U5.out[2] + U7_COMP_ENABLE * PSC_U7.out[2];
    ...
    NSC_U3.wo = FE_U1.out;
    NSC_U3.e[0] = PSC_U1.e[0];
    NSC_U3.e[1] = PSC_U1.e[1];
    NSC_U3.e[2] = PSC_U1.e[2];
    SEQ_CORE_MACRO(NSC_U3); // Ejecucion de macro de terc arm. (sec negativa)

    PSC_U3.wo = FE_U1.out;
    PSC_U3.e[0] = PSC_U1.e[0];
    PSC_U3.e[1] = PSC_U1.e[1];
    PSC_U3.e[2] = PSC_U1.e[2];
    SEQ_CORE_MACRO(PSC_U3); // Ejecucion de macro de terc arm. (sec positiva)

    NSC_U7.wo = FE_U1.out;
    NSC_U7.e[0] = PSC_U1.e[0];
    NSC_U7.e[1] = PSC_U1.e[1];
    NSC_U7.e[2] = PSC_U1.e[2];
    SEQ_CORE_MACRO(NSC_U7); // Ejecucion de macro de septimo arm. sec negativa

    PSC_U5.wo = FE_U1.out;
    PSC_U5.e[0] = PSC_U1.e[0];
    PSC_U5.e[1] = PSC_U1.e[1];
    PSC_U5.e[2] = PSC_U1.e[2];
    SEQ_CORE_MACRO(PSC_U5); // Ejecucion de macro de quinto arm. (sec popsitiva)


    NSC_U5.wo = FE_U1.out;
    NSC_U5.e[0] = PSC_U1.e[0];
    NSC_U5.e[1] = PSC_U1.e[1];
    NSC_U5.e[2] = PSC_U1.e[2];
    SEQ_CORE_MACRO(NSC_U5); // Ejecucion de macro de quinto arm. (sec negativa)

    PSC_U7.wo = FE_U1.out;
    PSC_U7.e[0] = PSC_U1.e[0];
    PSC_U7.e[1] = PSC_U1.e[1];
    PSC_U7.e[2] = PSC_U1.e[2];
    SEQ_CORE_MACRO(PSC_U7); // Ejecucion de macro de septimo arm.
    ...
    Grid.U = PSC_U1.Magn.out * 1; //  Conv.Vrng; // Actualizacion de valores medidos de la red electrica.
    ...
    // int mi_cmpb = PROJECT_PERIOD * (1 - (Grid.U * sin(Grid.theta + 0.5 * M_PI) + offet_Va) / 3.3);

    // EPwm4Regs.CMPB = mi_cmpb; // Mostramos el vector en el pin 7

    // Esto es para que CodeComposer no de warnings
    miU = miU;
    miF = miF;
    miT = miT;
    ...
    // Para sacar el vector rotante total, hay que sumar las componentes de todos los vectores rotantes
    // Hay que sumar componentes x E y. Las x directamente son variable.out[0]; las y son variable.Magn.out * variable.S.out[0] // v.C.out[0] es el coseno de cada angulo;
    // float componente_x = PSC_U1.Magn.out + NSC_U1.Magn.out + NSC_U5.Magn.out + PSC_U7.Magn.out;
    // float componente_y = PSC_U1.Magn.out * PSC_U1.S.out[0] + NSC_U1.Magn.out * NSC_U1.S.out[0] + NSC_U5.Magn.out * NSC_U5.S.out[0] + PSC_U7.Magn.out * PSC_U7.S.out[0];

    // float magnitud_total = sqrt(componente_x * componente_x + componente_y * componente_y);
    // float theta_total = atan2(componente_y, componente_x) +  2 * PI;
    ...
    // Devolvemos los valores de interes
    salida.f = Grid.f;
    salida.theta = Grid.theta;
    salida.mod = PSC_U1.Magn.out;
    salida.va = PSC_U1.out[0] + uuun * NSC_U1.out[0] + uuu3 * PSC_U3.out[0] + uun3 * NSC_U3.out[0] + uuu5 * PSC_U5.out[0] + uun5 * NSC_U5.out[0] + uuu7 * PSC_U7.out[0] + uun7 * NSC_U7.out[0]; // uuu3 * PSC_U3.out[0]
    salida.vb = PSC_U1.out[1] + uuun * NSC_U1.out[1] + uuu3 * PSC_U3.out[1] + uun3 * NSC_U3.out[1] + uuu5 * PSC_U5.out[1] + uun5 * NSC_U5.out[1] + uuu7 * PSC_U7.out[1] + uun7 * NSC_U7.out[1]; // uuu3 * PSC_U3.out[1]
    salida.vc = PSC_U1.out[2] + uuun * NSC_U1.out[2] + uuu3 * PSC_U3.out[2] + uun3 * NSC_U3.out[2] + uuu5 * PSC_U5.out[2] + uun5 * NSC_U5.out[2] + uuu7 * PSC_U7.out[2] + uun7 * NSC_U7.out[2]; // uuu3 * PSC_U3.out[2]
