...
// Control tensión
float kp_control_v = 10; // Calculado: 0.01
float ki_control_v = 0.2; // Calculado: 0.11

float Vbus_sp = 60;    // Set point de tension
...
// Adquisicion de senales simuladas
    ...
    // Para medir la duración de la interrupcion
    GPIO08_CLEAR;
}
...
CONVERTER_STATE run_converter_mef(PLL_VARS grid, float vbus)
{
    ...
    case CONVERTER_PRECHARGING:
    {
        ...
        if (cuenta_precarga > 2)
        {
            ...
            // Luego de 1 segundo abro el relé de precarga
            if (cuenta_aux > 20000)
            {
                PRECHARGE_OPEN;
    ...
    case CONVERTER_STARTING_KP:
    {
        ...
        */
        ...
        if ((Vbus_sp * 0.92 < vbus && vbus < Vbus_sp * 1.08) && a == 0)
        {
            cuenta_transicion++;
        }
        else
