 ...
 * 
 *      rev 4: se le dan los ultimos retoques al proyecto para su entrega
 *
 * Dependencias:
...
int stop_without_discharge  = 0;    // Apaga la maquina sin descargar el bus de CC
int stop_with_discharge     = 0;    // Apaga la maquina descargando el bus de CC
...
int main(void)
{
    ...
    gpio_init();// Initialize the shared GPIO pins (FILE: Gpio.c)
    ...
    while (1)
    {
        // Variables utilizadas para control y debug

        cac_state   = BOARD_GET_CAC1_STATE;
        cdc_state   = BOARD_GET_CDC1_STATE;
        qac_state   = BOARD_GET_QAC1_STATE;
        qdc_state   = BOARD_GET_QDC1_STATE;
        relay_state = BOARD_GET_PRECHARGE_RELAY_STATE;
