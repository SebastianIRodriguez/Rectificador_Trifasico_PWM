/**********************************************************************
 * File: DefaultIsr_7.c -- Solution File for Lab 7
...
typedef struct
{
	int B1v; // Tension en el bus 1
	int B2v; // Tension en el bus 2
	int Wi;	 // Corriente en W
	int Uv;	 // Tension en U
	int Vi;	 // Corriente en V
	int Vv;	 // Tension en V
	int Ui;	 // Corriente en U
	int Wv;	 // Tension en W
	int Ti;	 // Corriente por T
	// int B1v; // Tension en el bus 1
	int Si; // Corriente en S
	int Yv; // Tension en Y
	int Ri; // Corriente en R
	int Zv; // Tension en Z
	// int B2v; // Tension en el bus 2
	int Rv; // Tension en R
	int Tv; // Tension en T
...
typedef struct
{
	...
	float Uv; // Tension en U
	float Vv; // Tension en V
	float Wv; // Tension en W

	float Rv; // Tension en R
	float Tv; // Tension en T
	float Sv; // Tension en S

	float B1v; // Tension en el bus 1
	float B2v; // Tension en el bus 2

	// float Ti; // Corriente por T
	// float Si; // Corriente en S
	// float Yv; // Tension en Y
	// float Ri; // Corriente en R
	// float Zv; // Tension en Z

	// float Vi; // Corriente en V
	// float Wi; // Corriente en W
	// float Ui; // Corriente en U
} F_Variables;

//---------------------------------------------------------------------
interrupt void ADCINT_ISR(void) // PIE1.6 @ 0x000D4A  ADCINT (ADC)
{
	GpioDataRegs.GPATOGGLE.bit.GPIO18 = 1;	// Se hace un toggle del pin 18
	PieCtrlRegs.PIEACK.all = PIEACK_GROUP1; // Must acknowledge the PIE group
	...
	var.Wi = AdcRegs.ADCRESULT0 >> 4;
	var.Uv = AdcRegs.ADCRESULT1 >> 4;
	var.Vi = AdcRegs.ADCRESULT2 >> 4;
	var.Vv = AdcRegs.ADCRESULT3 >> 4;
	var.Ui = AdcRegs.ADCRESULT4 >> 4;
	var.Wv = AdcRegs.ADCRESULT5 >> 4;
	var.Ti = AdcRegs.ADCRESULT6 >> 4;
	var.B1v = AdcRegs.ADCRESULT7 >> 4;
	var.Si = AdcRegs.ADCRESULT8 >> 4;
	var.Yv = AdcRegs.ADCRESULT9 >> 4;
	var.Ri = AdcRegs.ADCRESULT10 >> 4;
	var.Zv = AdcRegs.ADCRESULT11 >> 4;
	var.B2v = AdcRegs.ADCRESULT12 >> 4;
	var.Rv = AdcRegs.ADCRESULT13 >> 4;
	var.Tv = AdcRegs.ADCRESULT14 >> 4;
	var.Sv = AdcRegs.ADCRESULT15 >> 4;

	//fvar.Wv = (var.Wv - OFFSET_WV) * KWV;
	//fvar.Uv = (var.Uv - OFFSET_UV) * KUV;
	//fvar.Vv = (var.Vv - OFFSET_VV) * KVV;
	//fvar.Rv = (var.Rv - OFFSET_RV) * KRV;
	//fvar.Sv = (var.Sv - OFFSET_SV) * KSV;
	//fvar.Tv = (var.Tv - OFFSET_TV) * KTV;
	//fvar.B1v = (var.B1v - OFFSET_B1V) * KB1;
	//fvar.B2v = (var.B2v - OFFSET_B2V) * KB2;
	 *
	 * */
	...
	int meda = AdcRegs.ADCRESULT0 >> 4;
	int medb = AdcRegs.ADCRESULT2 >> 4;
	int medc = AdcRegs.ADCRESULT4 >> 4;

	// Tomamos las variables correspondientes
	va = ((meda - offset_a) * VIN_MAX) / 4095.0;
	// ia = AdcRegs.ADCRESULT1 >> 4;
	vb = ((medb - offset_b) * VIN_MAX) / 4095.0;
	// ib = AdcRegs.ADCRESULT3 >> 4;
	vc = ((medc - offset_c) * VIN_MAX) / 4095.0;
...
/*
//--- Filtro PB
	static float v_out = 0;
	static float pi = 3.14159265;       // Famosa constante
	float fc = 10;                      // Frecuencia de corte
	float fs = 10000;                   // Frecuencia de muestreo
	float tau = 1/(2*pi*fc);            // Constante de tiempo
	float T = 1.0/fs;                   // Per�odo de muestreo
	float v_in;                         // Tensi�n muestreada
	unsigned int dc;                    // Ciclo de trabajo
	float parseada;                     // Tensi�n de entrada medida en muestras

	parseada = AdcRegs.ADCRESULT0 >> 4;
	v_in =  (parseada * VIN_MAX) / 4095.0;                              // Tensi�n muestreada
	v_out = (v_in * T - v_out * (T - tau))/tau;                         // Aplicamos el filtro digital
	dc = (PROJECT_PERIOD  -  (v_out * PROJECT_PERIOD) / VOUT_MAX) ;     // Ciclo de trabajo

	EPwm1Regs.CMPA.half.CMPA = dc;  // Pasamos el DC
*/
...
interrupt void EPWM1_TZINT_ISR(void) // PIE2.1 @ 0x000D50  EPWM1_TZINT (EPWM1)
{
	...
	// Se abren los contactores y reles xd
	...
	// Nos metemos en un bucle infinito para no hacer macanas
	while (1)
	{
		}
}
