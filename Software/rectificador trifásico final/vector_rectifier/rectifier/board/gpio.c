 ...
 * Description: Se definen las funciones para el manejo de los pines
 *              digitales y el pwm.
 *              Esta basado en el archivo del mismo nombre creado
 *              por Pablo Amoedo para el Lin-x
 *
 * History:
 *   10/01/2026 - original
...
#include "gpio.h"
#include <DSP2833x_Device.h> // Necesario para las macros de los registros
 ...
 * Retorno:     No posee
 *
 * Descripcion: Inicializa todos los pines digitales salvo los de ePWM.
 *              Los siguientes pines se configuran como salidas y se
 *              ponen en estado bajo:
 *
 *              GPIO 00 <GA>: EPWM1A
 *              GPIO 01 <GA>: EPWM1B
 *              GPIO 02 <GA>: EPWM2A
 ...
 *              GPIO 05 <GA>: EPWM3B
 *              GPIO 10 <GA>: (PWM2T-H)
 *              GPIO 11 <GA>: (Usada para debugging)
 *              GPIO 32 <GB>: (ISO-GPO2)
 *              GPIO 34 <GB>: (ISO-GPO3)
 *              GPIO 87 <GC>: (ISO-GPO1)
 *
 *              El resto de los pines es tratado como entrada digital y
 *              poseen resistencia de pull-up habilitada
 *
 **********************************************************************/
void gpio_init()
{
	...
	/// ---------------- Grupo A: Configuracion por defecto de Lin-X
	...
	GpioCtrlRegs.GPAPUD.all         = 0x00000000;   // Pullups habilitadas para todos los pines
	...
	GpioCtrlRegs.GPAMUX1.bit.GPIO6  = 1;    // 0=GPIO  1=EPWM4A     2=EPWMSYNCI  3=EPWMSYNCO    EPWM2R - H segunn la placa
	GpioCtrlRegs.GPAMUX1.bit.GPIO7  = 1;    // 0=GPIO  1=EPWM4B     2=MCLKRA     3=ECAP2        EPWM2R - L segunn la placa
	GpioCtrlRegs.GPAMUX1.bit.GPIO8  = 0;    // 0=GPIO  1=EPWM5A     2=CANTXB     3=ADCSOCAO     EPWM2S - H segunn la placa
	GpioCtrlRegs.GPAMUX1.bit.GPIO9  = 0;    // 0=GPIO  1=EPWM5B     2=SCITXDB    3=ECAP3        EPWM2S - L segunn la placa
	...
	// GpioCtrlRegs.GPADIR.bit.GPIO12 = 0; 		// 0 = Input (trip zone habilitado) Creo que no hace falta
	...
	GpioCtrlRegs.GPAMUX2.bit.GPIO20 = 1; // 0=GPIO  1=EQEP1A     2=MDXA       3=CANTXB
	GpioCtrlRegs.GPAMUX2.bit.GPIO21 = 1; // 0=GPIO  1=EQEP1B     2=MDRA       3=CANRXB
	...
	GpioCtrlRegs.GPAMUX2.bit.GPIO24 = 2; // 0=GPIO  1=ECAP1      2=EQEP2A     3=MDXB
	GpioCtrlRegs.GPAMUX2.bit.GPIO25 = 2; // 0=GPIO  1=ECAP2      2=EQEP2B     3=MDRB
	GpioCtrlRegs.GPAMUX2.bit.GPIO26 = 2; // 0=GPIO  1=ECAP3      2=EQEP2I     3=MCLKXB
    ...
    GpioCtrlRegs.GPADIR.bit.GPIO2 = 1;   // 1 = Output
    GpioCtrlRegs.GPADIR.bit.GPIO3 = 1;   // 1 = Output
    GpioCtrlRegs.GPADIR.bit.GPIO4 = 1;   // 1 = Output
    GpioCtrlRegs.GPADIR.bit.GPIO5 = 1;   // 1 = Output
    GpioCtrlRegs.GPADIR.bit.GPIO8  = 1; // 1 = Output
    GpioCtrlRegs.GPADIR.bit.GPIO9 = 1;  // 1 = Output
    GpioCtrlRegs.GPADIR.bit.GPIO10 = 1;  // 1 = Output
    GpioCtrlRegs.GPADIR.bit.GPIO11  = 1;        // 1 = Output
    ...
    GpioDataRegs.GPACLEAR.bit.GPIO4 = 1; // Output LOW
    GpioDataRegs.GPACLEAR.bit.GPIO5 = 1; // Output LOW

	// Grupo B: Configuracion por defecto de Lin-X
	...
	GpioCtrlRegs.GPBDIR.bit.GPIO32 = 1;	 // 1 = Output
	...
	// Grupo C: Configuracion por defecto de Lin-X
	...
	GpioCtrlRegs.GPCDIR.bit.GPIO87 = 1;	 // 1 = Output
	...
	// Configuracion de pines para la aplicacion

	//--- Selected pin configurations
	GpioDataRegs.GPCSET.bit.GPIO87 = 1; // GPO1
	GpioDataRegs.GPBSET.bit.GPIO32 = 1; // GPO2
	GpioDataRegs.GPBSET.bit.GPIO34 = 1; // GPO3, CAC1
	GpioDataRegs.GPASET.bit.GPIO10 = 1; // EPWM6, PWM2T-H OJO CON ESTO
	GpioDataRegs.GPCSET.bit.GPIO84 = 1; // Conv1-out1
 ...
 * Retorno:     No posee
 *
 * Descripcion: Modifica el mux para que los GPIOs 0-5 se conecten a su
 *              modulo ePWM correspondiente
 **********************************************************************/
 ...
 * Retorno:     No posee
 *
 * Descripcion: Modifica el mux para que los GPIOs 0-5 se desvinculen
 *              de los modulos ePWM y actuen como entradas digitales
 *              (alta impedancia)
 **********************************************************************/
void gpio_disable_epwm()
{
    ...
    GpioCtrlRegs.GPADIR.bit.GPIO0 = 0;  // Alta impedancia
    GpioCtrlRegs.GPADIR.bit.GPIO1 = 0;  // Alta impedancia
    GpioCtrlRegs.GPADIR.bit.GPIO2 = 0;  // Alta impedancia
    GpioCtrlRegs.GPADIR.bit.GPIO3 = 0;  // Alta impedancia
    GpioCtrlRegs.GPADIR.bit.GPIO4 = 0;  // Alta impedancia
    GpioCtrlRegs.GPADIR.bit.GPIO5 = 0;  // Alta impedancia
