/**********************************************************************
 * File: Adc_6_7_8.c -- Solution File for Labs 6, 7 and 8 (not used in Lab 5)
...
/**
 * @brief Inicializa el ADC de diversas maneras según sea el valor de test_num
 *
 * 	1: ADC canal 0 con SOC manejado por ePWM2, interrupción activa
 *
 * @param test_num parámetro que indica como se debe comportar el ADC
 */
void InitAdc(int test_num)
{
	switch (test_num)
	{
	case 1:
		// ADC canal 0 con SOC manejado por ePWM2, interrupción activa
		adc1();
		break;

	case 2:
		// ADC canal 0 con SOC manejado por ePWM4, interrupción activa
		adc2();
		break;

	case 4:
		// ADC en muestreo simultáneo
		adc4();
		break;

	case 5:
		// ADC en muestreo simultáneo en todos los canales utilizados
		adc5();
		break;

	case 6:
		// ADC en muestreo simultáneo, no genera interrupción
		adc6();
		break;
...
/**
 * @brief Configura el ADC para muestrear el canal 0 y tomar la señal de SOC desde el ePWM2
 * Además, habilita la interrupción al finalizar la secuencia 1 (SEQ1)
 *
 */
void adc1()
{
...
/**
 * @brief Configura el ADC para muestrear el canal A0 y tomar la señal de SOC desde el ePWM4
 * Además, habilita la interrupción al finalizar la secuencia 1 (SEQ1)
 *
 */
void adc2()
 ...
 * @brief Configura el ADC para muestrear el canal A0 y tomar la señal de SOC desde el ePWM4
 * Además, habilita la interrupción al finalizar la secuencia 1 (SEQ1)
 * Además, el ADC está en modo de muestreo simultáneo. Por ende toma la señal del A0 y B0.
 *
 */
void adc4()
{
...
void adc5()
{
	...
	// ***** Indicamos la cantidad de conversiones analógicas que se realizarán por secuencia
	AdcRegs.ADCMAXCONV.all = 0x0003;
	// bit 15-7      0's:    reserved
	// bit 6-4       000:    MAX_CONV2 value
	// bit 3-0       0000:   MAX_CONV1 value (0 means 1 conversion)

	// ***** Asignamos los canales de conversión al secuenciador
	AdcRegs.ADCCHSELSEQ1.bit.CONV00 = 0; // Convert Channel 0
	...
	// ********************* ACÁ SE CONFIGURA EL PERIODO DE ADQUISICION
	AdcRegs.ADCTRL1.all = 0x0710;
	// bit 15        0:      reserved
	// bit 14        0:      RESET, 0=no action, 1=reset ADC
	// bit 13-12     00:     SUSMOD, 00=ignore emulation suspend
	...
	// ********************* ACÁ SE CONFIGURA LA FUENTE DE SOC
	AdcRegs.ADCTRL2.all = 0x0900;
	// bit 15        0:      ePWM_SOCB_SEQ, 0=no action
	// bit 14        0:      RST_SEQ1, 0=no action
	// bit 13        0:      SOC_SEQ1, 0=clear any pending SOCs
...
/**
 * @brief Configura el ADC para muestrear el canal 0, 1, 2, 3, 4, 5 y 6 
 * y tomar la señal de SOC desde el modulo ePWM
 *
 */
void adc6()
{
